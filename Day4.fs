...
let toPassports input =
    Regex.Split (input, splitPattern, RegexOptions.Multiline)
    |> Array.map trim

let pKey = anyString 3 .>> pchar ':'
let pValue = manySatisfy (function ' ' -> false | '\n' -> false | _ -> true) .>> spaces
let pKeyValue = pKey .>>. pValue

type Passport = Map<string, string>
let pPassport = many pKeyValue

let parse input =
    run pPassport input
    |> function
        | Success (p, _, _) -> Map.ofList p
        | Failure (m, _, _) -> 
            printfn "There was an error: %s" m
            Map.empty
 
let verify (passport: Passport) =
    Map.containsKey "byr" passport &&
    Map.containsKey "iyr" passport &&
    Map.containsKey "eyr" passport &&
    Map.containsKey "hgt" passport &&
    Map.containsKey "hcl" passport &&
    Map.containsKey "ecl" passport &&
    Map.containsKey "pid" passport

let validate key validator (passport: Passport) =
    passport
    |> Map.tryFind key
    |> function
        | Some v -> validator v
...
let yearValidator min max year =
    match (Regex.Match (year, "\d{4}")) with
    | m when m.Success -> 
        let parsed = int m.Value
        parsed >= min && parsed <= max
    | _ -> false

let byrValidator value =
    yearValidator 1920 2002 value

let iyrValidator value =
    yearValidator 2010 2020 value

let eyrValidator value =
    yearValidator 2020 2030 value

let hgtValidator value =
    match (Regex.Match (value, "(\d+)(cm|in)")) with
    | m when m.Success ->
        let value = int m.Groups.[1].Value
        let unit = m.Groups.[2].Value
        if unit = "cm" then value >= 150 && value <= 193
        else value >= 59 && value <= 76
    | _ -> false

let hclValidator value =
    let m = Regex.Match (value, "#(\d|[a-f]){6}$")
    m.Success

let eclValidator value =
    List.contains value ["amb";"blu";"brn";"gry";"grn";"hzl";"oth"]

let pidValidator value =
    let m = Regex.Match (value, "^0*\d{9}$")
    m.Success


let verify2 passport =
    List.fold (fun (isValid, passport) validator -> 
        (isValid && validator passport, passport)) 
        (true, passport) 
        [validate "byr" byrValidator;
         validate "iyr" iyrValidator;
         validate "eyr" eyrValidator;
         validate "hgt" hgtValidator;
         validate "hcl" hclValidator;
         validate "ecl" eclValidator;
         validate "pid" pidValidator]
    |> fst
