...
let policyValidator password =
    let count = List.filter (fun curr -> curr = password.Policy.Letter) (Seq.toList password.Value) |> List.length
    count <= password.Policy.Max && count >= password.Policy.Min

let tobogganPolicyValidator password =
    let firstMatch = (List.item (password.Policy.Min - 1) (Seq.toList password.Value)) = password.Policy.Letter
    let secondMatch = (List.item (password.Policy.Max - 1) (Seq.toList password.Value)) = password.Policy.Letter
    firstMatch <> secondMatch

let solve isValid input =
    List.filter isValid input
    |> List.length

let pMinMax = pint32 .>>. (skipAnyChar >>. pint32)
let pPolicy = pipe2 (pMinMax .>> spaces) anyChar (fun (min, max) letter -> { Min = min; Max = max; Letter = letter})
let pPassword: Parser<Password, unit> = pipe2 pPolicy (skipChar ':' >>. spaces >>. restOfLine true) (fun policy value -> { Policy = policy; Value = value })

let getPasswords input =
    run (many pPassword) input
    |> function
        | Success (ps, _, _) -> ps
        | Failure _ -> []

let input = "1-8 n: dpwpmhknmnlglhjtrbpx
