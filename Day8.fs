...
let pAccLine = (skipString "acc" >>. pOpValue |>> Accumulator) .>> spaces
let pJumpLine = (skipString "jmp" >>. pOpValue |>> Jump) .>> spaces

let pOperations = many (choice [pNopLine; pAccLine; pJumpLine])

let parseInput input =
    match run pOperations input with
    | Success (operations, _, _) -> { state with Operations = operations }
    | Failure _ -> state

let updateHistory state =
    Map.add state.Pointer 1 state.History

let rec solve state =
    match Map.tryFind state.Pointer state.History with
    | Some _ -> state.Counter
