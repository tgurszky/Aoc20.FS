...
let incrementOrAdd (map: Map<char, int>) value =
    match Map.containsKey value map with
    | true -> Map.change value (Option.map increment) map
    | false -> Map.add value 1 map

let toMap (group: string) =
    Seq.toList group
    |> List.fold incrementOrAdd Map.empty

let solve (input: string) =
    toGroups input
    |> Array.sumBy (toSet >> Set.count)

let toCountAndMap (group: string) =
    (group.Split('\n') |> Array.length, toMap group)

let matchingCount (count, map) =
    Map.fold (fun state _ value -> if value = count then (state + 1) else state) 0 map

let solve2 (input: string) =
    toGroups input
    |> Array.sumBy (toCountAndMap >> matchingCount)
